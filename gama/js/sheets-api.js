...
// IDs de las hojas de cálculo (EXACTAMENTE como en el GAS)
...
class SheetsAPI {
    ...
    // Función equivalente a Sheets.Spreadsheets.Values.get()
    ...
    // FUNCIÓN PRINCIPAL - EXACTA AL GAS
    async obtenerDatosCombinados() {
        try {
            console.time('Carga_GAS_Exacta');
            console.log('🚀 Cargando datos EXACTA al GAS...');
            
            // Obtener datos de todas las fuentes (EXACTO AL GAS)
            const datosData2 = await this.obtenerDatosDeData2();
            const datosSiesa = await this.obtenerDatosSiesa();
            const datosData = await this.obtenerDatosDeDataConSumas();
            const datosSoportes = await this.obtenerDatosSoportes();
            const datosRec = await this.obtenerDatosRec();
            const datosGlobales = await this.obtenerDatosGlobales();
            
            // Combinar datos de DATA2 y REC (EXACTA LÓGICA AL GAS)
            const datosCombinados = [];
            
            // 1. PROCESAR DATOS DE DATA2 (EXACTO AL GAS)
            datosData2.forEach(itemData2 => {
                ...
                if (coincidenciasSiesa.length > 0) {
                    ...
                    // NO TIENE FACTURAS - PROCESAR DISTRIBUCIÓN (ENTREGAS SIN FACTURA)
                    datosRelacionados = [];
            ...
            // 2. PROCESAR DATOS DE REC (EXACTO AL GAS)
            ...
            datosRec.forEach(filaRec => {
                ...
                // Verificar si ya existe en datosCombinados
            ...
            console.timeEnd('Carga_GAS_Exacta');
            
            // ESTADÍSTICAS DETALLADAS
            const totalRegistros = datosCombinados.length;
            const registrosConFactura = datosCombinados.filter(item => 
                item.datosSiesa && item.datosSiesa.some(siesa => siesa.factura && siesa.factura.trim() !== '')
            ).length;
            const registrosSinFactura = datosCombinados.filter(item => 
                item.datosSiesa && item.datosSiesa.some(siesa => !siesa.factura || siesa.factura.trim() === '')
            ).length;
            
            console.log(`📊 ESTADÍSTICAS GAS EXACTO:`);
            console.log(`   Total registros: ${totalRegistros}`);
            console.log(`   Con factura: ${registrosConFactura}`);
            console.log(`   Sin factura: ${registrosSinFactura}`);
            console.log(`   DATA2: ${datosCombinados.filter(d => d.fuente === 'DATA2').length}`);
            console.log(`   REC: ${datosCombinados.filter(d => d.fuente === 'REC').length}`);
    ...
    // FUNCIONES AUXILIARES - EXACTAS AL GAS

    async obtenerDatosDeDataConSumas() {
        try {
            const allValues = await this.fetchSheetData(SPREADSHEET_IDS.DATA, 'DATA!C:C');
            ...
            for (let i = 0; i < allValues.length; i++) {
                ...
                try {
                    ...
                    console.error(`Error parseando JSON en fila ${i+1}:`, e);
                }
            }
            
            return datosPorDocumento;
        } catch (error) {
            console.error("Error al obtener datos de DATA:", error);
            return {};
        }
    }
    ...
    async obtenerDatosDeData2() {
        try {
            const allValues = await this.fetchSheetData(SPREADSHEET_IDS.DATA2, 'DATA2!S:S');
            ...
            console.error("Error al obtener datos de DATA2:", error);
            return [];
        }
    }
    ...
    async obtenerDatosRec() {
        try {
            const allValuesRec = await this.fetchSheetData(SPREADSHEET_IDS.REC, 'DataBase!A2:AB');
            ...
            console.error("Error al obtener datos de REC:", error);
            return [];
        }
    }
    ...
    async obtenerDatosSoportes() {
        try {
            const allValuesSoportes = await this.fetchSheetData(SPREADSHEET_IDS.SOPORTES, 'SOPORTES!A:G');
            const soportesMap = {};
            
            for (let i = 1; i < allValuesSoportes.length; i++) {
                const row = allValuesSoportes[i];
                if (row.length >= 7) {
            ...
            console.error("Error al obtener datos de SOPORTES:", error);
            return {};
        }
    }
    ...
    async obtenerDatosGlobales() {
        try {
            const data = await this.fetchSheetData(SPREADSHEET_IDS.REC, 'DataBase!A:HR');
            if (!data) return [];
            ...
            const cantidadesDIS = {};
            try {
                const disData = await this.fetchSheetData(SPREADSHEET_IDS.DIS, 'DIS.!A:N');
                disData.forEach(fila => {
                    const doc = fila[8];
                    if (!doc) return;
                    cantidadesDIS[doc] = {
                        "805027653": fila[10] || "",
    ...
    async obtenerDatosSiesa() {
        try {
            const allValuesSiesa = await this.fetchSheetData(SPREADSHEET_IDS.SIESA, 'SIESA!A:G');
            const allValuesSiesaV2 = await this.fetchSheetData(SPREADSHEET_IDS.SIESA, 'SIESA_V2!A:D');
            ...
            console.error("Error al obtener datos de SIESA:", error);
            return [];
        }
    }
...
// Para compatibilidad con app.js existente
