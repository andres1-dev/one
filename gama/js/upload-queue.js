...
class UploadQueue {
  ...
  this.queue.forEach((item, index) => {
    ...
    if (item.type === 'photo') {
      previewContent = `Factura: ${item.factura || 'N/A'} | Documento: ${item.data.documento || 'N/A'}`;
        ...
        thumbnail = `
          <div class="thumbnail-container">
            <img src="data:image/jpeg;base64,${item.data.fotoBase64}" 
                 ...
                 alt="Vista previa" 
                 style="display: block;">
            <div class="thumbnail-overlay">Vista previa</div>
          </div>
        `;
      } else {
        thumbnail = '<div class="no-thumbnail">Sin vista previa disponible</div>';
      }
