...
async function actualizarFilaEspecifica(rec) {
    ...
    try {
        ...
        // Buscar datos existentes para evitar reprocesamiento
        const datosExistentes = documentosGlobales.find(d => d.rec === rec);
        ...
        const documentoActualizado = {
            ...
            fecha_objeto: datosExistentes ? datosExistentes.fecha_objeto : parsearFecha(fechaSolo),
            cantidad: datosExistentes ? datosExistentes.cantidad : 0,
            lote: datosExistentes ? datosExistentes.lote : '',
            refProv: datosExistentes ? datosExistentes.refProv : '',
            prenda: datosExistentes ? datosExistentes.prenda : '',
            tieneClientes: datosExistentes ? datosExistentes.tieneClientes : false,
            datosCompletos: datosExistentes ? datosExistentes.datosCompletos : null,
        ...
        // Recalcular consolidados de forma optimizada
        recalcularConsolidadosOptimizado();
...
// Función para actualización inmediata en UI
function actualizarUIInmediatamente(rec, responsable) {
    if (!documentosTable) return;
    
    // Actualizar inmediatamente en la tabla local
    const fila = documentosTable.row((idx, data) => data.rec === rec);
    if (fila.any()) {
        const datosActuales = fila.data();
        datosActuales.colaborador = responsable;
        fila.data(datosActuales).draw(false);
        
        // Actualizar el select en la UI
        const rowNode = fila.node();
        const selectCell = $(rowNode).find('td:eq(2)');
        selectCell.html(generarSelectResponsables(rec, responsable, documentosGlobales, datosActuales));
    }
    
    // Actualizar documentosGlobales
    const index = documentosGlobales.findIndex(d => d.rec === rec);
    if (index !== -1) {
        documentosGlobales[index].colaborador = responsable;
    }
}

// Función para revertir UI en caso de error
function revertirUI(rec) {
    if (!documentosTable) return;
    
    // Forzar recarga de esa fila específica
    setTimeout(() => {
        actualizarFilaEspecifica(rec);
    }, 100);
}

// Función optimizada para actualización posterior al cambio
async function actualizacionSelectivaPostCambio(rec) {
    try {
        // 1. Actualizar SOLO los datos necesarios para impresión
        await actualizarDatosImpresion(rec);
        
        // 2. Actualizar la fila específica (sin recargar toda la tabla)
        await actualizarFilaEspecifica(rec);
        
        // 3. Recalcular consolidados de forma optimizada
        recalcularConsolidadosOptimizado();
        
    } catch (error) {
        console.error('Error en actualización selectiva:', error);
        // En caso de error, recargar solo esa fila
        await actualizarFilaEspecifica(rec);
    }
}

// Función optimizada para actualizar datos de impresión
async function actualizarDatosImpresion(rec) {
    try {
        // Actualizar solo el documento específico en datosGlobales
        const SPREADSHEET_ID = "1d5dCCCgiWXfM6vHu3zGGKlvK2EycJtT7Uk4JqUjDOfE";
        const API_KEY = 'AIzaSyC7hjbRc0TGLgImv8gVZg8tsOeYWgXlPcM';
        ...
        if (rowData && typeof cargarDatos === 'function') {
            // Actualizar solo este documento en datosGlobales
            const documentoIndex = datosGlobales.findIndex(doc => doc.REC === rec);
            if (documentoIndex !== -1) {
                // Mantener los datos existentes pero actualizar el responsable
                datosGlobales[documentoIndex].RESPONSABLE = rowData[4] || '';
            }
        }
        
        return true;
    } catch (error) {
        console.error('Error actualizando datos impresión:', error);
        return false;
    }
}

// Función optimizada para recalcular consolidados
function recalcularConsolidadosOptimizado() {
    if (!documentosTable) return;
    
    // Usar datos ya filtrados de la tabla
...
// Función ALTAMENTE OPTIMIZADA para cambiar responsable
async function cambiarResponsable(rec, responsable) {
    ...
    try {
        ...
        actualizarUIInmediatamente(rec, responsable);
        
        // 2. Llamar a la API (solo esto es bloqueante)
        ...
        if (result.success) {
            // 3. Actualización selectiva y diferida
            await actualizacionSelectivaPostCambio(rec);
            
            await mostrarNotificacion('✓ Asignado', responsable, 'success');
            console.log(`Responsable ${responsable} asignado a REC${rec}`);
