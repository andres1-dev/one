...
async function cambiarResponsable(rec, responsable) {
    try {
        ...
        // Mostrar loading
        Swal.fire({
            title: 'Asignando responsable...',
            text: `Asignando ${responsable} a REC${rec}`,
...
async function cambiarEstadoDocumento(rec, nuevoEstado) {
    try {
        ...
        Swal.fire({
            ...
            text: `Cambiando estado de REC${rec} a ${nuevoEstado}`,
            allowOutsideClick: false,
            didOpen: () => {
                Swal.showLoading();
            }
...
async function imprimirSoloClientesDesdeTabla(rec) {
    try {
        ...
        Swal.fire({
            ...
            text: `Cargando datos de REC${rec}`,
            allowOutsideClick: false,
            didOpen: () => {
                Swal.showLoading();
            }
        ...
        if (typeof imprimirSoloClientes === 'function') {
            ...
            await mostrarNotificacion('Éxito', `Preparando impresión de REC${rec}`, 'success');
        } else {
