...
// Función para mostrar indicador de actualización sutil
function mostrarIndicadorActualizacion(mensaje = 'Actualizando...') {
    const indicador = document.getElementById('indicador-actualizacion');
    if (indicador) {
        indicador.innerHTML = `<i class="fas fa-sync-alt fa-spin me-2"></i>${mensaje}`;
...
async function actualizarInmediatamente(mostrarLoader = false) {
    try {
        if (mostrarLoader) {
            mostrarIndicadorActualizacion('Sincronizando cambios...');
        }

        // Guardar estado actual de la tabla
        const paginaActual = documentosTable ? documentosTable.page() : 0;
        const ordenActual = documentosTable ? documentosTable.order() : [[0, 'desc']];
        const busquedaActual = documentosTable ? documentosTable.search() : '';

        // Recargar datos
        await cargarTablaDocumentos(true, paginaActual, ordenActual, busquedaActual);
...
// Función para actualización automática en background
async function actualizacionAutomatica() {
    try {
        console.log('🔄 Actualización automática en background...');
        
        // Verificar si han pasado suficientes segundos desde la última actualización manual
        if (ultimaActualizacion) {
            const tiempoTranscurrido = new Date() - ultimaActualizacion;
            if (tiempoTranscurrido < 10000) { // Menos de 10 segundos
                console.log('⏭️ Saltando actualización automática (actualización reciente)');
                return;
            }
        }
...
function iniciarActualizacionAutomatica() {
    ...
    autoRefreshInterval = setInterval(actualizacionAutomatica, AUTO_REFRESH_INTERVAL);
...
function actualizarTimestampUI() {
    ...
    if (timestampElement && ultimaActualizacion) {
        ...
        if (segundos < 5) {
            ...
            texto = `Hace ${segundos} seg`;
        } else {
            const minutos = Math.floor(segundos / 60);
            texto = `Hace ${minutos} min`;
        }
        
        timestampElement.textContent = texto;
    }
}

// Actualizar timestamp cada segundo
setInterval(actualizarTimestampUI, 1000);
...
function actualizarTarjetasResumen(consolidados) {
    const elementos = {
        pendientes: document.getElementById('contadorPendientes'),
        pendientesUnidades: document.getElementById('unidadesPendientes'),
        proceso: document.getElementById('contadorProceso'),
        procesoUnidades: document.getElementById('unidadesProceso'),
        directos: document.getElementById('contadorDirectos'),
        directosUnidades: document.getElementById('unidadesDirectos'),
        total: document.getElementById('contadorTotal'),
        totalUnidades: document.getElementById('unidadesTotal')
    };
    
    // Función helper para animar cambios
    function animarCambio(elemento, nuevoValor) {
        if (!elemento) return;
        
        const valorActual = elemento.textContent;
        if (valorActual !== nuevoValor.toString()) {
    ...
    animarCambio(elementos.pendientes, consolidados.pendientes.count);
    if (elementos.pendientesUnidades) elementos.pendientesUnidades.textContent = `${consolidados.pendientes.unidades} unidades`;
    
    animarCambio(elementos.proceso, consolidados.proceso.count);
    if (elementos.procesoUnidades) elementos.procesoUnidades.textContent = `${consolidados.proceso.unidades} unidades`;
    
    animarCambio(elementos.directos, consolidados.directos.count);
    if (elementos.directosUnidades) elementos.directosUnidades.textContent = `${consolidados.directos.unidades} unidades`;
    
    animarCambio(elementos.total, consolidados.total.count);
    if (elementos.totalUnidades) elementos.totalUnidades.textContent = `${consolidados.total.unidades} unidades`;
}
...
// Función para configurar filtro de fecha
...
// Función para cargar la tabla de documentos (MEJORADA)
async function cargarTablaDocumentos(mantenerEstado = false, paginaActual = 0, ordenActual = [[0, 'desc']], busquedaActual = '') {
    try {
        const esPrimeraVez = !documentosTable;
        ...
        if (documentosTable && !mantenerEstado) {
        ...
        if (mantenerEstado && documentosTable) {
            // Actualizar datos sin destruir la tabla
            documentosTable.clear();
            documentosTable.rows.add(documentosDisponibles);
            documentosTable.page(paginaActual).draw(false);
            documentosTable.order(ordenActual).draw(false);
            if (busquedaActual) {
                documentosTable.search(busquedaActual).draw(false);
            }
            
            // Reiniciar timers
            iniciarTimers(documentosDisponibles);
            
            // Reaplicar eventos a los nuevos elementos
            $('.select-responsable').off('change').on('change', function() {
            ...
            // Iniciar actualización automática solo la primera vez
            if (esPrimeraVez) {
...
// Función para cambiar responsable (CON ACTUALIZACIÓN INMEDIATA)
async function cambiarResponsable(rec, responsable) {
    try {
        console.log(`🔄 Asignando responsable ${responsable} a REC${rec}`);
        mostrarIndicadorActualizacion(`Asignando responsable a REC${rec}...`);
        ...
        if (result.success) {
            mostrarMensajeToast(`✅ Responsable actualizado: ${responsable}`, 'success');
...
// Función para cambiar estado del documento (CON ACTUALIZACIÓN INMEDIATA)
async function cambiarEstadoDocumento(rec, nuevoEstado) {
    try {
        console.log(`🔄 Cambiando estado del documento REC${rec} a: ${nuevoEstado}`);
        mostrarIndicadorActualizacion(`Cambiando estado de REC${rec}...`);
        ...
        if (result.success) {
            ...
            mostrarMensajeToast(`✅ Estado cambiado a ${nuevoEstado}`, 'success');
...
// Función para restablecer documento (CON ACTUALIZACIÓN INMEDIATA)
async function restablecerDocumento(rec) {
    ...
    if (password === 'one') {
        try {
            ...
            mostrarIndicadorActualizacion(`Restableciendo REC${rec}...`);
            ...
            if (result.success) {
                ...
                mostrarMensajeToast(`✅ Documento REC${rec} restablecido`, 'success');
...
// Función para mostrar mensajes toast (notificaciones sutiles)
function mostrarMensajeToast(mensaje, tipo = 'info') {
    const icono = {
    ...
    toast.innerHTML = `
        <div style="display: flex; align-items: center; gap: 10px;">
            ...
            <i class="fas fa-times" style="cursor: pointer; opacity: 0.5;" onclick="this.parentElement.parentElement.remove()"></i>
        </div>
    `;
...
function mostrarError(mensaje) {
    mostrarMensajeToast(mensaje, 'error');
}
...
// Detectar cuando la pestaña está visible/oculta para pausar/reanudar actualización
document.addEventListener('visibilitychange', function() {
    if (document.hidden) {
        console.log('⏸️ Pestaña oculta - pausando actualización automática');
        detenerActualizacionAutomatica();
    } else {
        console.log('▶️ Pestaña visible - reanudando actualización automática');
        iniciarActualizacionAutomatica();
