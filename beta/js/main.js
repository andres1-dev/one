...
class ConfigManager {
  ...
  setupEventListeners() {
    ...
    document.getElementById('scannerToggle').addEventListener('change', (e) => {
      this.scannerEnabled = e.target.checked;
      this.saveSettings();
    });
    ...
    // Input de barcode - siempre captura input físico (lector láser)
    ...
    // Prevenir focus no deseado
    barcodeInput.addEventListener('focus', (e) => {
      if (!this.keyboardEnabled) {
        // Solo prevenir el teclado virtual, no el input físico
        e.target.blur();
      }
    });
  ...
  loadSettings() {
    try {
      ...
      if (saved) {
        ...
        this.scannerEnabled = settings.scannerEnabled !== undefined ? settings.scannerEnabled : true;
  ...
  applySettings() {
    ...
    document.getElementById('scannerToggle').checked = this.scannerEnabled;
